...
高溫作業',
    ...
    items: [
      ...
      { id: 'fev1_fvc', label: 
'FEV1/FVC', unit: '%', type: 'number', ref: '>=70', checkType: 'all' },
      ...
      { id: 'na', label: 
'鈉 (Na)', unit: 'mmol/L', type: 'number', ref: REF_DATA.na, checkType: 'all' },
      ...
      { id: 'urine_bld', 
label: '尿潛血', type: 'select', options: ['(-)', '(+/-)', '(+)', '(++)'], ref: '(-)', checkType: 'all' },
    ]
  },
  '02': {
    name: '02.
噪音作業',
    ...
    items: [
      ...
      { id: 'r_avg', label: '右耳平均(0.5-2k)', 
unit: 'dB', type: 'number', ref: '<25', checkType: 'all' },
    ]
  },
  '03': {
    name: '03.
...
const femalePart = parts.find(p => p.trim().startsWith('F:'));
    if (malePart && femalePart) {
       targetRef = gender === '男' ?
malePart.replace('M:', '').trim() : femalePart.replace('F:', '').trim();
    }
  }
...
useEffect(() => {
  ...
  specificItems.filter(item => {
            ...
            if (!newCase.itemResults[item.id]) defaultResults[item.id] = { value: 
'', isAbnormal: false };
        });
...
if (field === 'value') {
        ...
        if (itemId === 'fev1_fvc' || itemId === 'fvc' || itemId === 'fev1') {
            const getVal = (key) => key === itemId ?
parseFloat(value) : parseFloat(newCase.itemResults[key]?.value || 0);
      ...
      <header className="bg-teal-700 text-white shadow-lg sticky top-0 z-50">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
          <div className="flex items-center space-x-2">
            <img src="image_0.png" alt="安澤健康顧問" className="h-8 mr-3" />
            <h1 className="text-xl font-bold tracking-wide">安澤健康顧問 - 勞工特殊健檢管理系統</h1>
          </div>
          <nav 
...
(
         ...
         <div className="grid grid-cols-4 gap-4">
                                ...
                                <div className="text-center p-3 bg-blue-50 rounded border border-blue-100"><div className="text-xs text-blue-600 mb-1">第二級</div><div className="text-xl font-bold 
text-blue-700">{data.g2}</div></div>
          ...
          <div className="max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-lg border border-gray-200">
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded border border-gray-200">
                    <div><label className="block text-xs font-bold text-gray-500 mb-1">姓名</label><input type="text" value={newCase.name} onChange={e => 
setNewCase({...newCase, name: e.target.value})} className="w-full p-2 border rounded"/></div>
...
font-bold">1. 曾經從事</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                        <div className="md:col-span-1 text-sm font-bold">2.
特殊作業項目</div>
                        <div className="md:col-span-3">
                            <select className="w-full p-1 border rounded text-sm font-bold text-teal-700" value={newCase.hazardType} onChange={e=>setNewCase({...newCase, hazardType: e.target.value})}>
                                {HAZARD_TYPES_LIST.map(t => <option key={t.id} 
value={t.id}>{t.name}</option>)}
                            </select>
                        </div>
                        <div className="md:col-span-8 flex items-center gap-2 text-sm"><span>起始:</span><input type="month" value={newCase.currentWorkStart} onChange={e=>setNewCase({...newCase, currentWorkStart: e.target.value})} className="border rounded p-1"/><span>截至:</span><span className="bg-gray-200 p-1 rounded">檢查日</span><span>共</span><input type="text" placeholder="年/月" value={newCase.currentWorkYears} onChange={e=>setNewCase({...newCase, currentWorkYears: e.target.value})} className="w-20 
border rounded p-1"/></div>
                    </div>
                    <div className="flex items-center gap-2 text-sm"><span className="font-bold">3.
平均每日工時：</span><input type="number" value={newCase.dailyHours} onChange={e=>setNewCase({...newCase, dailyHours: e.target.value})} className="w-20 border rounded p-1"/> 小時</div>
                </div>
            </div>
 ...
 {BASIC_ITEMS.map(item => {
                        const res = newCase.itemResults[item.id] ||
{ value: '', isAbnormal: false };
                   ...
                   <label className="text-xs text-gray-500">{item.label} {item.unit ? `(${item.unit})` : ''}</label>
                                ...
                                {item.type === 'select' ? (
                                    ...
                                    <input type="text" value={res.value} onChange={e => handleItemResultChange(item.id, 'value', e.target.value, item)} className={`w-full p-1 text-sm border rounded ${res.isAbnormal ?
'text-red-600 font-bold' : ''}`} placeholder={item.ref} />
                                )}
                            </div>
                        )
            ...
            <div className="bg-teal-50 p-6 rounded-lg border border-teal-200 mb-8">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-bold text-teal-900 flex 
items-center gap-2"><Stethoscope className="w-5 h-5"/> 2. 特殊檢查項目</h3>
                    <div className="flex gap-2">
                        <span className="text-xs bg-white px-2 py-1 rounded text-teal-600 border border-teal-200">{currentHazardInfo.name}</span>
                        <span className="text-xs bg-teal-600 px-2 py-1 rounded text-white border border-teal-600">{isEntryCheck ?
'體格檢查' : '健康檢查'}</span>
                    </div>
                </div>
...
(
                                ...
                                <div className="col-span-2 text-center text-xs text-gray-500 bg-gray-50 rounded py-1">{item.ref ||
'-'}</div>
                                <div className="col-span-2 flex items-center"><label className={`w-full flex items-center justify-center cursor-pointer px-2 py-2 rounded transition-colors text-sm ${res.isAbnormal ?
'bg-red-100 text-red-700 font-bold border border-red-200' : 'bg-gray-50 text-gray-400 hover:bg-gray-200'}`}><input type="checkbox" checked={res.isAbnormal} onChange={(e) => handleItemResultChange(item.id, 'isAbnormal', e.target.checked, item)} className="hidden" />{res.isAbnormal ?
'異常' : '正常'}</label></div>
                            </div>
                        );
})}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             ...
             <label className="text-sm font-bold text-gray-700 block mb-2">管理分級判定</label>
                    <div className="bg-gray-50 p-4 rounded border border-gray-200">
                        <div className="flex gap-2 mb-3">{['1', '2', '3', '4'].map(g => (<button key={g} onClick={() => setNewCase({...newCase, grade: g})} className={`flex-1 py-2 rounded font-bold transition-all ${newCase.grade === g ? (g==='1'?'bg-green-600 text-white':g==='2'?'bg-blue-600 text-white':g==='3'?'bg-yellow-500 text-white':'bg-red-600 text-white') : 'bg-white border border-gray-300 
text-gray-600 hover:bg-gray-100'}`}>{g} 級</button>))}</div>
        ...
        {activeTab === 'list' && selectedEmp && (
            <div className="flex flex-col items-center">
               ...
               <div ref={reportRef} className="p-10 text-gray-900 relative h-full flex flex-col text-sm">
                        <div className="text-center mb-4"><h1 className="text-2xl font-bold border-b-2 border-gray-800 inline-block pb-1 mb-1">{HAZARD_DB[selectedEmp.hazardType]?.name ||
selectedEmp.hazardType} 勞工特殊體格及健康檢查紀錄</h1></div>
             ...
             <div>檢查日期: {selectedEmp.checkDate}</div>
                        ...
                        <div className="mb-4 border border-gray-400 p-2"><h3 className="font-bold mb-2 bg-gray-100 p-1">二、作業經歷</h3><p>1.
曾經從事: {selectedEmp.pastWork} ({selectedEmp.pastWorkStart} ~ {selectedEmp.pastWorkEnd}, 共 {selectedEmp.pastWorkYears})</p><p>2. 特殊作業項目: {HAZARD_DB[selectedEmp.hazardType]?.name} ({selectedEmp.currentWorkStart} ~ 至今, 共 {selectedEmp.currentWorkYears})</p><p>3.
平均每日工時: {selectedEmp.dailyHours} 小時</p></div>
       ...
       <div className="mb-4 border border-gray-400 p-2">
             ...
             <div><strong className="block border-b mb-1">基本項目</strong>{BASIC_ITEMS.map(item => (<div key={item.id} className="flex justify-between border-b border-dashed"><span>{item.label}</span><span>{selectedEmp.itemResults[item.id]?.value || '-'}</span></div>))}</div>
                                <div><strong className="block border-b mb-1">特殊項目</strong>{(HAZARD_DB[selectedEmp.hazardType]?.items || []).filter(item => { const isEntry = selectedEmp.checkType === '新進員工(受僱時)';
if(item.checkType === 'all') return true; if(isEntry && item.checkType === 'entry') return true; if(!isEntry && item.checkType === 'periodic') return true;
return false; }).map(item => (<div key={item.id} className="flex justify-between border-b border-dashed"><span>{item.label}</span><span className={selectedEmp.itemResults[item.id]?.isAbnormal ? 'text-red-600 font-bold' : ''}>{selectedEmp.itemResults[item.id]?.value || '-'}{selectedEmp.itemResults[item.id]?.isAbnormal && ' (異)'}</span></div>))}</div>
                            </div>
                        </div>

                        <div className="mb-4 border border-gray-400 
p-2"><h3 className="font-bold mb-2 bg-gray-100 p-1">五、健康管理與醫師建議</h3><div className="mb-2"><strong>管理分級: </strong><span className="text-xl font-bold mx-2">第 {selectedEmp.grade} 級管理</span><span className="text-sm text-gray-500">({GRADE_RECOMMENDATIONS[selectedEmp.grade].desc})</span></div><div className="mb-2"><strong>醫師建議: </strong> {selectedEmp.doctorNote}</div><div><strong>應處理及注意事項: </strong><ul className="list-disc pl-5 mt-1 text-xs text-gray-700">{GRADE_RECOMMENDATIONS[selectedEmp.grade].actions.map((act, i) => <li key={i}>{act}</li>)}</ul></div></div>
      ...
      {/* IMPORT */}
        {activeTab === 'import' && (
             ...
             <div className="bg-white p-12 rounded-lg shadow-lg border-2 border-dashed border-gray-300 hover:border-teal-500 transition-colors group">
               ...
               <h3 
className="text-xl font-medium text-gray-700 mb-2">批次匯入健檢資料</h3>
 ...
 <div className="text-center"><button onClick={downloadTemplate} className="text-teal-600 hover:text-teal-800 underline flex items-center justify-center mx-auto gap-2" disabled={!isXlsxLoaded}><FileSpreadsheet className="w-4 h-4"/>{isXlsxLoaded ?
'下載範本格式 (Template)' : '正在載入工具...'}</button></div>
           </div>
        )}
      </main>
    </div>
