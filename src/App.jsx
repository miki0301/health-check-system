...
export default function OccupationalHealthApp() {
  ...
  // -----------------------------------------------------------------------------
  // ★ 修復閃退 BUG：補上渲染所需的衍生變數定義
  // -----------------------------------------------------------------------------
  const currentHazardInfo = HAZARD_DB[newCase.hazardType] || { name: '請選擇作業類別', items: [] };
  const isEntryCheck = newCase.checkType === '新進員工(受僱時)';
  ...
  const abnormalCount = Object.values(newCase.itemResults).filter(res => res && res.isAbnormal).length;
  // -----------------------------------------------------------------------------
